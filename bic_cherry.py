...
def get_species(node) -> tuple[str, int]:
    node_attributes = dict(node.attributes())
    #(label, species)
    return (node_attributes['label'], node_attributes['reconc'])

def build_species_color_map(tree, colors: list[str]) -> dict:
    species_color_map = dict()
    for node in tree.leaves():
        _, node_species = get_species(node)

        if node_species not in species_color_map.keys():
            species_color_map[node_species] = colors.pop(0)
    return species_color_map

def init_graph(tree, colors) -> nx.DiGraph:
    graph = nx.DiGraph(name="bic_cherry")
    graph.add_node("r", color="black", kind='root', layer=3, label="Root")

    for node in tree.leaves():
        node_label, node_species = get_species(node)
        graph.add_node(node_label, color=colors[node_species], kind='leaf', layer=0, label=node_label)
    return graph
